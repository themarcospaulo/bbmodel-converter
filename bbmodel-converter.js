...
const modelsPath = path.join(__dirname, 'models/entity');

function readPath(modelsFolder) {
  const files = fs.readdirSync(modelsFolder);
  let results = [];

  files.forEach(file => {
    const fullPath = path.join(modelsFolder, file);
    const stat = fs.statSync(fullPath);

    if (stat.isDirectory()) {
      results = results.concat(readPath(fullPath));
    } else {
...
function getAllModels() {
  return readPath(modelsPath).filter(file => path.extname(file) === '.bbmodel');
}

getAllModels().forEach(file => {
  const data = fs.readFileSync(file, 'utf8');
  const name = path.basename(file, '.bbmodel');

  const resultName = path.join(__dirname, 'models/entity', name + '.geo.json');
  
  console.log(`Converting ${name}.bbmodel into ${name}.geo.json`);
  
  fs.writeFileSync(
    resultName,
    exportModel(JSON.parse(data), name)
  );

  fs.unlinkSync(file);
});

function exportModel(data, name) {
  let entitymodel = {};
  let main_tag = {
  ...
  entitymodel.description = {
    identifier: "geometry." + (data.model_identifier || name),
